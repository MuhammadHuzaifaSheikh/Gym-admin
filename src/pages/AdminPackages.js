    ...
    useEffect(() => {
        const fetchJim = async () => {
            try {
                ...
                console.log('gymee',response.data.data)
                let tempGym = response.data.data?.businessLocations?.results.filter((item)=>  response.data.data?.packages.some((v)=>v?._id ===item._id   )             )
