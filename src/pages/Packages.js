  ...
  return (
    <>
      <div className="container-xxl flex-grow-1 container-p-y">
        ...
        <PackagesModel show={showModal} handleClose={handleCloseModal} getPackagesList={getPackagesList} />
        <div class="row">
                        ...
                        <div class="flex-grow-1">
                          <div class="row">
                            <div class="col-md-8">
                              ...
                              <div class="text-muted mb-1 d-flex flex-wrap"><span class="me-1">Duration</span> <a href="javascript:void(0)" class="me-3">Monthly</a> {user.isAdmin || filterPackages == "mypackages" ? (<></>) : (<span class="badge bg-label-success" style={{ cursor: "pointer" }} onClick={() => SubscribePackage(item._id.toString())}>Subscribe Package</span>)}</div>
                              <div class="text-muted mb-1 d-flex flex-wrap"><span class="me-1">Description: {item.description}</span> </div>
                            </div>
                            <div class="col-md-4">
                              <div class="text-md-end">
                                <div class="my-2 my-lg-4"><span class="text-primary">${item.price}</span></div>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <li class="list-group-item p-4">
                  <div class="d-flex gap-3">
                    <div class="flex-grow-1">
                      <div class="row">
                        <div class="col-md-8">
                          <div class="text-muted mb-1 d-flex flex-wrap"><span class="me-1">No available Package to show</span> </div>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <ul className="pagination">
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageno) => (
                        <li key={pageno} className={`page-item ${page === pageno ? 'active' : ''}`}>
                          <button className="page-link" onClick={() => handlePageChange(pageno)}>{pageno}</button>
                        </li>
                      ))}
                    </ul>
                  </nav>
            ...
            {activePackage ? (
              <div class="border rounded p-4 pb-3">
                ...
                <dl class="row mb-0">
                  ...
                  <dd class="col-6 fw-medium text-end text-heading mb-0">${activePackage.price}</dd>
                </dl>
              </div>
            ) : (
              <div class="border rounded p-4 pb-3">
